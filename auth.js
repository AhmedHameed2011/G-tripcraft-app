...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!sbClient) {
    console.error('❌ Supabase client is not initialized. Ensure supabase-config.js is loaded prior to auth.js.');
    return;
  }
  ...
  function updateAuthUI(session) {
    ...
    if (!authNavGroup || !userNavGroup) {
      console.warn('⚠️ Navigation user groups missing in DOM.');
      return;
    }
    ...
    if (session && session.user) {
      ...
      userNavGroup.innerHTML = `
        <div class="user-profile-badge" style="display: flex; align-items: center; gap: 0.5rem; background: var(--bg-secondary, rgba(150,150,150,0.1)); padding: 0.25rem 0.75rem 0.25rem 0.25rem; border-radius: 50px;">
          <div id="userAvatar" style="width: 32px; height: 32px; border-radius: 50%; background: var(--primary, #007bff); color: white; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 14px;">
            ${initial}
          </div>
          <span id="userNameDisplay" style="font-weight: 500; font-size: 0.9rem; color: var(--text-primary, #ffffff);">${displayName}</span>
        </div>
        <button id="btnLogout" class="btn btn-secondary" type="button" style="padding: 0.4rem 0.75rem; display: flex; align-items: center; gap: 0.25rem;">
  ...
  if (registerForm) {
    registerForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        const msg = data?.session ? 'Registration successful! Welcome.' : 'Registration successful! Please check your email to confirm.';
        if (typeof window.showToast === 'function') {
